...
/**
 * RawMoto — vitrine publiée + domaine myshopify vérifié.
 * Pas de HTTP prod, pas de slugs (`rawmoto`), pas d’origines inventées.
 */
...
export type StorefrontFrameEnvironment = "production" | "development";

export function resolveStorefrontFrameEnvironment(
  nodeEnv: string | undefined = process.env.NODE_ENV,
  appEnv: string | undefined = process.env.NEXT_PUBLIC_APP_ENV,
): StorefrontFrameEnvironment {
  if (appEnv === "production") return "production";
  if (appEnv === "development" || appEnv === "preview") return "development";
 ...
 */
export function buildStorefrontFrameAncestors(
  environment: StorefrontFrameEnvironment = resolveStorefrontFrameEnvironment(),
): string {
  const ancestors: string[] = [
    "'self'",
    ...STOREFRONT_SELF_HOSTS,
    ...RAWMOTO_FRAME_ANCESTORS,
  ];
...
export function buildStorefrontContentSecurityPolicy(
  environment?: StorefrontFrameEnvironment,
): string {
  const frameAncestors = buildStorefrontFrameAncestors(environment);
